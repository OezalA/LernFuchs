...
**German** (her native/school language) and in **English as a foreign language**.
A "Üben:" switch in the header picks which language to practise; everything else
(games, progress, streak) stays the same.
...
> The *learning content* is either German or English:
> - **German** — full immersion: words, example sentences and the meaning
>   explanations are all in child-friendly German.
> - **English (foreign language)** — deliberately easier (A1–A2): short texts and
>   simple questions in English, but each new word is **explained in German**
>   (native-language help). English texts also offer an optional word-learning step
>   (flash-cards with pronunciation) *before* reading, so the child isn't overwhelmed.
>
...
- **VocabularyWord** — a word with article (der/die/das), plural, word type, a
  child-friendly German definition (for English words: the German translation),
...
The UI (in German) has a start page with a **progress dashboard**, a **"Üben:"
language switch** (Deutsch / Englisch) in the header, and three areas:
...
- **Leseverständnis** — read a short text (with optional **read-aloud**), filter by
  ...
  The **difficult words are extracted into the Wortschatz**. In **English** mode a
  text first offers an optional **word-learning step** (flash-cards with English
  pronunciation → German meaning) so new words are learned before reading.
